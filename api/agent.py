...
def run_agent(task: str):
    ...
    # Increase char limit to 2500 so the agent gets enough context on the first search
    api_wrapper = WikipediaAPIWrapper(top_k_results=2, doc_content_chars_max=2500)
    search_tool = WikipediaQueryRun(api_wrapper=api_wrapper)
    tools = [search_tool]

    # Instruct the agent to cap searches at 2 and synthesize immediately
    system_prompt = """You are an Autonomous Research Agent. 
    When given a research task, follow this workflow:
    1. PLAN: Identify 1 or 2 broad Wikipedia search terms related to the topic.
    2. ACT: Use the wikipedia tool to search (perform AT MOST 2 searches total).
    3. OBSERVE: Review the retrieved Wikipedia summaries.
    4. RESPOND: Immediately synthesize a comprehensive, well-structured final report with headers, bullet points, and a summary based on the findings. Do not perform more than 2 searches."""
